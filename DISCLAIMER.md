...
| **歌词（唯一的外发路径，默认关闭）** | 见下方 §2.1。 |

> 补充说明：项目仓库中**不包含**任何读取或解密本机其它应用凭据的代码。源码中 `AuthMode.localClient` 仅作为能力枚举声明存在，**没有任何实现**。

### 2.1 联网歌词：唯一一处会对外发送数据的功能

歌词有两个来源，**都不涉及网盘账号数据**：

| 来源 | 默认 | 实际做法 |
|---|---|---|
| **本地 `.lrc`**（优先） | 开启 | 直接读取用户自己网盘中、与音乐同目录的 `.lrc` 文件。正文在本机索引库中，不对外发送任何内容。 |
| **LRCLIB 联网匹配** | **关闭** | 仅在用户在「设置 → 歌词」中**明确开启**后生效。这是本应用唯一一处会向网盘之外的第三方发送数据。 |

开启联网歌词后，播放一首本地没有 `.lrc` 的曲目时，客户端会向 [LRCLIB](https://lrclib.net)
（`lrclib.net`，一个公开的歌词库，无需注册、服务端开源可自托管）发送该曲目的
**曲名 / 艺术家 / 专辑名 / 时长**，用于匹配歌词。

- **不发送**：音频文件本身、网盘文件列表、播放记录、收藏、授权凭证，以及任何可用于定位用户网盘账号的标识。
- **不经过本项目作者控制的任何服务器** —— 该请求由客户端直接发往 LRCLIB。
- 匹配到的歌词正文**写入本机 SQLite 索引库**（这是本应用唯一一处将第三方文本写入本地库的地方），
  与本地 `.lrc` 的正文同等对待；**不向任何地方回传**。
- 关闭开关后**不再发起新的匹配请求**；在此之前已取到的正文仍留在本机，用户可随时通过「清空曲库」一并删除。
- **不开启该开关，本应用不会与 LRCLIB 发生任何通信。** 该设置的读取与写入见 `lib/data/db/settings_store.dart`，
  默认值逻辑见 `lib/ui/providers/lyrics_providers.dart` —— 均可自行审计。

> 歌词文本本身的著作权归其权利人所有。本项目不提供、不缓存分发、不转码任何歌词内容，
> 仅在用户自己的设备上展示其自己选择获取的文本；用户应自行确保其使用符合适用法律。
...
- 本项目**不提供、不缓存、不转码、不分发**任何音频内容。所有音频数据始终存放在用户自己的网盘中，由网盘服务器直接向用户的播放器传输。（唯一例外是歌词正文在用户开启相应功能后会存于本机索引库，见 §2.1。）
