...
| **收藏** | 喜欢列表独立于网盘，纯本地 |
| **自动发现新歌** | 后台按 2 小时（±30 分钟抖动）不定期重新扫描网盘；新进库的歌会在「音乐库」侧栏项上亮红色数字徽标、曲目标题旁标「新」；点曲库页头的「标记 N 首已看」即可清掉 |
...
后台会**不定期**重新扫描你已授权的网盘，把新进来的歌挑出来、提示你。

**扫描节奏**：默认每 **2 小时**扫一轮，带 **±30 分钟**随机抖动 —— 这样所有客户端不会在同一秒去打接口，避免被限流。授权状态变化时自动启停：登录后才开始、登出或注销后停下。只扫已授权的网盘。

**怎么算「新」**：每首歌记下它**第一次被索引到的时间**（`first_seen_at`，数据库列，和索引时间分开）。一首歌「新不新」= 它的进库时间是否晚于你**上次看完新歌**的时刻（水位）。水位没建立（比如升级后第一次启动）时，整库**不标新** —— 否则一打开全是红点，毫无意义。基线由扫描服务在**第一次成功扫描后**写入，所以老库不会被当成新歌。

**提示出现在两处：**

- 「音乐库」侧栏项右上角的**红色数字徽标**（`N` 首新歌）；
- 曲目标题旁的蓝色 **「新」** 小标签。

点曲库页头的「**标记 N 首已看**」，把水位设为现在，徽标和标签一起消失。水位写进设置，重启后依然有效（这是「这批看过了」，不是「这次会话没新歌」）。

> 全程**只读你自己的网盘、不向外发任何东西**，和联网歌词不同 —— 它只是把「新文件出现了」这件事在本地标出来。
...
### 10. 「新歌」提示依赖后台扫描，且只认本地索引时间

- 红点 / 「新」标签**不是**实时监听网盘变化的推送，而是后台每 2 小时（±30 分钟）扫一轮、拿**本次扫描发现的新文件**来标记。网盘里刚上传一首歌，最快也要等下一轮扫描才会出现提示。
- 「新不新」看的是**第一次被本应用索引到的时间**，不是文件本身的修改时间。一首歌你早就在网盘里放了、但本应用是今天才第一次扫到它，它今天也会算「新」—— 这是设计使然，避免老库被一次性刷成满屏新歌。
- 应用退出（或登出）时后台扫描随之停止，下次启动并授权后才会恢复。
